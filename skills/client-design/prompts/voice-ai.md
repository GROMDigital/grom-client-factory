...
4. `<clientFolder>/design/phone-and-compliance.md`, IF IT EXISTS. It is opt-in as of 2026-07-28 and absent on most builds, because Grom executes phone compliance by hand. When it is missing, take the same two things from **registry section 8, "Phone, compliance, domains"**, which always exists, and keep every go-live gate you would otherwise have written: the gating is real whether or not a design doc spells it out. Never presume approval has landed just because the doc that described the approval is not there. When it does exist it gives you TWO things: the market compliance rules your prompts must obey (banned and approved language, consent basis, opt-out handling, calling windows) and the approval waits that gate go-live. Voice cannot go live on the tracked number until that compliance bundle is approved; your go-live section states this dependency explicitly.
