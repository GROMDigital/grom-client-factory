...
3. **BOOKING MODEL RECOMMENDATION.** Pick one: calendar page, in-page widget, AI-booked, or external. Give the reasoning from the offer and traffic shape. Grom's standard funnel routes the Meta lead form to a booking/deposit landing page where the lead books and pays, so assume a landing page hosts the booking mechanism unless the strategy explicitly books elsewhere. If the strategy is silent on booking intent, recommend a default and flag it in the adequacy verdict as a question.
