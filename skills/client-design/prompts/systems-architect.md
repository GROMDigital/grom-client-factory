...
- 🔴 A SKIPPED role gets NO doc-index row with a filename. Write "not this client" in the File column instead, exactly as you already do for voice AI. The full skip set is now `no_lps`, `no_voice`, `no_chat_ai`, `no_calendars`, `no_phone_compliance` and `no_domains_deliverability`, and the last three are new. This matters mechanically: the orchestrator asserts that every filename in the doc index exists on disk and STOPS the run when one does not, so a row naming a file for a role that was never run halts the build after the entire fan-out has been paid for. The default configuration skips two roles, so getting this wrong is the common case, not the edge case.
