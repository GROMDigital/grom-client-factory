...
`{client_key: <kebab-case>, no_lps: <bool>, no_voice: <bool>, no_chat_ai: <bool>, lps: [{slug, purpose}], workflows: [{number, name}], doc_index: [{file, owner_role}], summary_for_human: <a tight paragraph a human reads at the go/no-go gate: what you built, the key decisions, the biggest risks or gaps>, fields_and_tags_for_human: <section 5, restated in plain English>}`
