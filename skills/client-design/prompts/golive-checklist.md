...
Before anything else, read `baseline/guardrails.md` verbatim (your bootstrap gives you its absolute path) and treat every rule in it as absolute. Never name the platform in any lead-visible artifact this checklist references: it is always "the Grom system". No em dashes anywhere, internal or client-visible.
...
🔴 BOTH OF THOSE DOCS ARE OPT-IN as of 2026-07-28 and are ABSENT on most builds, because Grom executes phone compliance and domain setup by hand. Their absence is normal and is NOT a blocker. When either is missing, fall back to **registry section 8, "Phone, compliance, domains"**, which always exists and carries the tracked-number plan, the compliance bundle path for the market, the sending domain, the LP domain, and the allowed_origins values these produce. Build your day-1 items and your GATED-BY gates from section 8 instead, at the same level of specificity, and mark each such item's detail owner as "Grom, by hand, outside the design set" so nobody reads the shorter entry as an oversight. Never silently drop a gate because its source doc was not written: a checklist that loses its compliance and DNS gates is worse than one that says the detail lives elsewhere.
