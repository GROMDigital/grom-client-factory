...
3. THE LIVE TRACKING REPO README: resolve the local clone path from `~/.grom-factory.json` under `deps["client-lp-tracking"].path`, then read `<that path>/README.md` plus its `landing-pages/` and `booking-steps/` folders, for the real snippet contract, event hooks, and provisioning steps. Read this repo FRESH at run time: it is the authoritative rollout checklist for your role, and if its README has changed since this prompt was written, the README wins on process. Do not restate its steps in your doc; defer to it by that resolved path.
...
7. **Verification pointer.** State how a paste is confirmed live, deferring to the live tracking repo's checklist by its resolved clone path rather than restating the queries: after a real visit, the client's events must appear for that landing before the slice is considered installed. Note that the landing page itself is built outside this factory and is reconciled to THIS doc (its selectors, snippet config, and events) by the `grom-client-factory:reconcile-lp-tracking` skill before go-live; write your selectors and booking mode so that skill has an unambiguous target to check the built page against.
