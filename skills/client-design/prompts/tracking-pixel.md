...
7. **Verification pointer.** State how a paste is confirmed live, deferring to the live tracking repo's checklist by its resolved clone path rather than restating the queries: after a real visit, the client's events must appear for that landing before the slice is considered installed.
